...
def prepare_ray_datasets(dset: Union[TabularDataset, CachedDataset],
	...
	for split in dset.splits:
		# skip splits that we don't need
		...
		(split in ["new_train", "oracle"]):
			continue 
		if (split == "train" and split_train_loaders_by_domain) or \
		(split in ["id_test", "ood_test", "new_ood_test"] and \
			compute_per_domain_test_metrics):
			raise NotImplementedError(("renaming the datasets", 
				"needs to be updated for this to work"))
...
def run_ray_tune_experiment(dset: Union[TabularDataset, CachedDataset],
	...
	def train_loop_per_worker(config: Dict):
		...
		def _on_train_end(loss_train, epoch):
			"""Function to be called at the end of each training epoch to log
		...
		for epoch in range(n_epochs):
			...
			if epoch == 0:
				...
				_on_train_end(train_loss, epoch)
			else:
	...
	if is_pytorch_model_name(model_name):
		...
		datasets = prepare_ray_datasets(dset,
										...
										compute_per_domain_test_metrics=False)
		scaling_config = ScalingConfig(
			num_workers=tune_config.num_workers,
